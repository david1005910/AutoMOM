...
export class SttService {
  ...
  async transcribe(filePath: string): Promise<string> {
    if (isMockMode()) {
      ...
      await new Promise((r) => setTimeout(r, 1500)); // 처리 중인 척
    ...
    return this.transcribeInChunks(filePath);
  }
  ...
  private async transcribeInChunks(filePath: string): Promise<string> {
    const chunkDuration = 600; // 10분 청크
    const chunks = await this.splitAudio(filePath, chunkDuration);

    const results = await Promise.all(
      chunks.map((chunk) => this.transcribeFile(chunk.path, chunk.offset))
    );
  ...
  private splitAudio(
    ...
    return new Promise((resolve, reject) => {
      ffmpeg.ffprobe(filePath, async (err, metadata) => {
        ...
        const chunks: { path: string; offset: number; cleanup: () => Promise<void> }[] = [];
        const tmpFiles: tmp.FileResult[] = [];
